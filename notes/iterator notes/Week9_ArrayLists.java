...
class ArrayUtils {
	...
	<T, U> ArrayList<U> map(ArrayList<T> alist, Function<T, U> fun) {
		...
		return this.mapHelp(alist, fun, 0, result);
	}
	...
	<T> boolean containsSequence(ArrayList<T> source, ArrayList<T> sequence) {
		...
		while (sourceIndex < source.size() && sequenceIndex < sequence.size()) {
			if (source.get(sourceIndex).equals(sequence.get(sequenceIndex))) {
				sequenceIndex ++;
			}
...
class ExamplesArrayList {
	...
	void initData() {
		...
		this.ints2 = new ArrayList<Integer>();
	...
	boolean testContainsSequence(Tester t) {
		...
		return t.checkExpect(util.containsSequence(ints, ints2), true);
	}
