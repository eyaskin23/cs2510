...
class Cell {
  ...
  Cell(int x, int y, String color, boolean flooded, Cell left, 
...
class FloodItWorld extends World {
  ...
  FloodItWorld(ArrayList<Cell> board, int bdSize, int colors) {
    boardSize = bdSize;
    colorsUsed = colors;
    this.board = board;
    if (boardSize > 12) {
  ...
  public WorldScene makeScene() {
    ...
    finalScene.placeImageXY(
        ...
        700, 200);
    finalScene.placeImageXY(
        new TextImage("Welcome to Flood-IT! EY CS2510", 25, FontStyle.BOLD_ITALIC, Color.BLACK), 240,
        550);
    ...
    finalScene.placeImageXY(
        new TextImage("Time: " + time / 200 + "s", 25, FontStyle.BOLD_ITALIC, Color.black), 650,
        250);
    ...
    if (clicks >= required && (!allFlooded())) {
      finalScene.placeImageXY(new TextImage("YOU LOSE!", 25, Color.RED), 670, 300);
    }
    else if (clicks <= required && allFlooded()) {
      finalScene.placeImageXY(new TextImage("YOU WIN1", 25, Color.GREEN), 670, 300);
    }
    for (Cell c : board) {
  ...
  public void updateWorld() {
    ...
    ArrayList<Cell> alreadySeen = new ArrayList<>();
    // Initialize the worklist with the from the first cell
    worklist.add(floodingFromCell);
    // As long as the worklist isn't empty
    while (!worklist.isEmpty()) {
      // Remove the first element from the worklist
      Cell next = worklist.remove(0);
      if (next.flooded) {
        next.update(floodingTo);
      }
      if (next.flooded && !next.color.equals(floodingTo)) {
        next.setColor(floodingTo);
        break;
      }
      if (alreadySeen.contains(next)) {
        ...
        // add all the neighbor cells of next to the worklist for further processing
        for (Cell neighbor : next.getNeighbors()) {
          if (neighbor.flooded) {
  ...
  void startGame(int gridSize, int numberOfColor) {
    if (numberOfColor > 8) {
...
class ExamplesFloodIt {
  ...
  // Initial condition
  void initWorld() {
    cBlue = new Cell(0, 0, "BLUE", true, null, null, null, null);
    cRed = new Cell(1, 0, "RED", false, cBlue, null, null, null);
    cGreen = new Cell(0, 1, "GREEN", false, null, cBlue, null, null);
    cYellow = new Cell(1, 1, "YELLOW", false, cGreen, cRed, null, null);
    cOrange = new Cell(0, 2, "ORANGE", false, null, cGreen, null, null);
    cBlack = new Cell(1, 2, "BLACK", false, cOrange, cYellow, null, null);
    cPink = new Cell(0, 3, "PINK", false, null, cOrange, null, null);
    cPurple = new Cell(1, 3, "PURPLE", false, cPink, cBlack, null, null);
    cBlue.right = cRed;
    cBlue.bottom = cGreen;
    cRed.bottom = cYellow;
  ...
  void testImage(Tester t) {
    initWorld();
    t.checkExpect(this.cBlue.image(), new RectangleImage(20, 20, OutlineMode.SOLID, Color.blue));
    t.checkExpect(this.cRed.image(), new RectangleImage(20, 20, OutlineMode.SOLID, Color.red));
    t.checkExpect(this.cGreen.image(), new RectangleImage(20, 20, OutlineMode.SOLID, Color.green));
    ...
    t.checkExpect(this.cBlack.image(), new RectangleImage(20, 20, OutlineMode.SOLID, Color.black));
    t.checkExpect(this.cPink.image(), new RectangleImage(20, 20, OutlineMode.SOLID, Color.pink));
  ...
  void testUpdateWorld(Tester t) {
    ...
    // exWorld2.updateWorld2();
    t.checkExpect(exWorld2.board.get(1).color, "GREEN");
    t.checkExpect(exWorld2.board.get(2).color, "RED");
  }
